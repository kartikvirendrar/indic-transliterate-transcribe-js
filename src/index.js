    ...
    setVoiceLogs(prevLogs => {
      const updatedLogs = prevLogs.map(log => {
        ...
        if (changeStart > log.end) {
          ...
          newEnd += lengthDelta;
        } else if (changeStart > log.start && changeStart <= log.end) {
          newEnd += lengthDelta;
        }
        return {
          ...log,
          ...
          setVoiceLogs(prevLogs => {
            const updatedOldLogs = prevLogs.map(log => {
              if (log.start >= cursorPos) {
                return { ...log, start: log.start + transcriptLength, end: log.end + transcriptLength };
              }
              return log;
            });
            ...
            const newLogs = [...updatedOldLogs, newLog];
