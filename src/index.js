  ...
  const enableVoiceTyping = () => {
    ...
    target.addEventListener("input", function handler() {
      ...
      for (let i = 0; i < chunksSorted.length; i++) {
        ...
        if (chunkStart === -1) {
          chunkStart = currentValue.indexOf(thisChunk.asr_output, cursor);
        }
        if (chunkStart === -1) {
        ...
        if (nextChunk) {
          ...
          if (nextStart === -1)
            nextStart = currentValue.indexOf(nextChunk.asr_output, chunkStart + thisChunk.user_correction.length);
    ...
    setInterval(() => {
      if (voiceLogs.length) {
        const logsToSend = voiceLogs
          .filter(log => !log.deleted)
          .map(({audio_base64, asr_output, user_correction}) => ({
            audio_base64, asr_output, user_correction
          }));
